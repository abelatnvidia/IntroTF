...
label_ops = tf.one_hot(decode_ops[9],2)
...
params_init = tf.random_normal(shape=[param_sz, label_sz], stddev=1.0, name='model-params-init', dtype=tf.float32)
...
Y = tf.placeholder(tf.float32, shape=[batch_sz,label_sz], name='labels')

# with perfect parameters/model: records*params = labels
y = tf.add(tf.matmul(X, w),b, name='model-values')

error = tf.nn.softmax_cross_entropy_with_logits(labels=Y,logits=y)
...
with tf.Session() as sess:
    ...
    for i in range(100):
        ...
        sfw.add_summary(loss_summary,global_step.eval())
...
'''
    ...
    The only problem is ... optimization done not converge!
    Maybe our loss function isn't quite right ...
'''