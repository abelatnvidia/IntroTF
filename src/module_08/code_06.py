...
with tf.Session() as sess:
    ...
    # reshape the image
    image = sess.run(tf.reshape(image, [480, 640]))
...
'''
    Notice, it is really easy to swap the image dimensions.
    However, now it looks like we've only got a black and white image.
    What happened to the color image we started with?
