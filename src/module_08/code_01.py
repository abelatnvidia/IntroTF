...
'''
  Most of this is exactly the same as our previous example working with 
  notMNIST.  The only difference here is that we have list of bbox
  coordinates that are different for each example.  However, you see that
  actually, always encode the data as list of values (!) even when we only
  have a single peice of data to encode.     
'''